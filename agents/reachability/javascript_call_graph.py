...
class JavaScriptCallGraphBuilder:
        ...
        try:
            ...
            for i, line in enumerate(lines):
                ...
                if potential_func:
                    ...
                    scope_stack.append({'name': unique_name, 'level': brace_balance})
                    current_scope = unique_name
                ...
                if self._is_route_definition(line):
                    ...
                    NON_HANDLERS = frozenset({
                        'req', 'res', 'next', 'err', 'request', 'response',
                        'options', 'config', 'app', 'router', 'true', 'false', 'null',
                    })
                    handler_args = re.findall(r',\s*([a-zA-Z0-9_$]+)\s*(?=[,)])', line)
                    for h in handler_args:
                        if h not in NON_HANDLERS:
                            self.entry_points.add(h)
    ...
    def get_mermaid_graph(self, traces: List[List[str]]) -> str:
        ...
        def _label(node: str) -> str:
            return node.split(':', 1)[-1]  # strips "path/to/file.js:"
        ...
        for trace in traces:
            for i in range(len(trace) - 1):
                ...
                edge = f'    {src_id}["{_label(src)}"] --> {dst_id}["{_label(dst)}"];'
