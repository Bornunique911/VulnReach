    ...
    def _process_file(self, file_path: Path):
        ...
        try:
            ...
            for i, line in enumerate(lines):
                ...
                class_match = class_pattern.search(strip_line)
                if class_match:
                ...
                elif current_class and re.search(
                    rf'\b{re.escape(current_class)}\s*\([^)]*\)\s*\{{', strip_line
                ):
                    # Constructor: same name as class, no return type
                    ctor_name = f"{current_class}.<init>"
                    scope_stack.append({'name': ctor_name, 'level': brace_balance})
                    current_method = ctor_name
                    self.function_locations[ctor_name] = f"{rel_path}:{i+1}"
                    if ctor_name not in self.graph:
                        self.graph[ctor_name] = set()
                    # Constructors are entry points for DI-managed components
                    if any(a in {'@Component', '@Service', '@Repository', '@Bean'}
                           for a in pending_annotations):
                        self.entry_points.add(ctor_name)
                    pending_annotations.clear()
