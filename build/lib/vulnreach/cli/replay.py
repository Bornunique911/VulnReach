...
def replay(ctx: click.Context, cve_id: str, scan_id: str, fmt: str) -> None:
    ...
    if mode == "client":
        ...
        raw = scan.get("raw") or {}
        # Search all reachability tool outputs for this CVE
        for tool_name in ("python_reachability", "java_reachability", "multi_language_reachability"):
            payload = raw.get(tool_name) or {}
            for analysis in payload.get("analyses", []) or payload.get("vulnerabilities", []):
                cve_ids = analysis.get("cve_ids") or []
                if cve_id in cve_ids or analysis.get("cve_id") == cve_id:
                    graph_str = analysis.get("call_chain_graph")
                    package = analysis.get("package_name")
                    break
            if graph_str:
                break
